...
class SampleVideoEncodedFrameObserver
    : public agora::media::IVideoEncodedFrameObserver {
private:
...
public:
  ...
  bool
  OnEncodedVideoFrame(rtc::uid_t uid, const uint8_t *imageBuffer, size_t length,
...
public:
  ...
  bool onPlaybackAudioFrameBeforeMixing(const char *channelId, rtc::uid_t uid,
    ...
    UnifexEnv *env = unifex_alloc_env(NULL);
    UnifexPayload payload;
    unsigned int size = audioFrame.samplesPerChannel *
                        audioFrame.bytesPerSample * audioFrame.channels;
...
public:
  ...
  void
  onUserAudioTrackSubscribed(user_id_t userId,
...
UNIFEX_TERM create(UnifexEnv *env, char *appId, char *token, char *channelId,
  ...
  state->localUserObserver =
      std::make_shared<SampleLocalUserObserver>();
  ...
  state->connection->getLocalUser()->setPlaybackAudioFrameBeforeMixingParameters(2, 44100);
